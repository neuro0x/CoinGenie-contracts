 ...
 */
contract CoinGenieV2 is Payments {
    ...
    function setGenie(ICoinGenieERC20 genie_) public onlyOwner {
        ...
        launchedTokens.push(address(genie_));

        // Create a new LaunchedToken struct
        LaunchedToken memory genieToken = LaunchedToken({
            index: 0,
            tokenAddress: address(genie_),
            name: genie_.name(),
            symbol: genie_.symbol(),
            totalSupply: genie_.totalSupply(),
            feeRecipient: genie_.feeRecipient(),
            affiliateFeeRecipient: genie_.affiliateFeeRecipient(),
            taxPercent: genie_.taxPercent(),
            maxBuyPercent: genie_.maxBuyPercent(),
            maxWalletPercent: genie_.maxWalletPercent()
        });

        // Add the token details to the mapping of launched tokens
        launchedTokenDetails[address(genie_)] = genieToken;

        // Add the token to the array of tokens launched by the fee recipient
        tokensLaunchedBy[genie_.feeRecipient()].push(genieToken);

        // Add the token details to the mapping of launched tokens
        launchedTokenDetails[address(genie_)] = genieToken;

        // Set the user
        users.push(genie_.feeRecipient());
    }
